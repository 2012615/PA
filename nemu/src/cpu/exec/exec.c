...
typedef struct {
  DHelper decode;   //during the phase of decode?
  EHelper execute;  //during the phase of exec?
  int width;
} opcode_entry;


/*
#define concat_temp(x, y) x ## y  //##means connect the two words
...
/* 0x80, 0x81, 0x83 */
    ...
    make_group(gp5,
               ...
               EX(jmp_rm), EMPTY, EX(push), EMPTY)  //why rm????
    ...
    */
    ...
    // should define in make_group

    // R: general registers;
    // E: normal registers & memory
    // b: byte
    // v: word | double word | quadword-->due to the patern of cpu
    ...
    /*!!!!ATTENTION!!!  Mod R/M  used in mov?
        7        6 5             3 2      0
        ___________________________________
        |   Mod   |  Reg / Opcode |  R/M  |
        -----------------------------------
        maybe it is used in mov inst.
        Reg/Opcode determines which operande is general register
        Mod and R/M makes sure the E is a register or a addr of mem.
    */
    ...
    in this part, I try to insert endbr32 inst. into the opcode table, and treat it as
    a nop.
   ...
   procedure calls by reducing the number of instructions required to 
   retain the contents of the general registers for use in a procedure. 
   The processor pushes the general registers on the stack in the 
   following order: EAX, ECX, EDX, EBX, the initial value of ESP before 
   EAX was pushed, EBP, ESI, and EDI. PUSHA is complemented by the POPA 
   instruction
   */

    //jcc-->conditional-->means needs to read the value of reg
    //jmp-->unconditional
    ...
    CWDE (Convert Word to Doubleword Extended) extends the sign of the 
    ...
    */
    opcode_entry opcode_table[512] = {
        ...
        /* 0x30 */ IDEXW(G2E, xor, 1), IDEX(G2E, xor), IDEXW(E2G, xor, 1), IDEX(E2G, xor), // not sure!!!!
        ...
        /* 0x54 */ IDEX(r, push), IDEX(r, push), IDEX(r, push), IDEX(r, push), // r means the mod field of the modR/M byte may refer only to a general register;
        ...
        /* 0xe8 */ IDEX(J, call), IDEX(J,jmp), EMPTY, IDEXW(J,jmp,1), // call
        ...
        /* 0x90 */ IDEXW(E,setcc,1), EMPTY, IDEXW(E,setcc,1), EMPTY,
        /* 0x94 */ IDEXW(E,setcc,1), IDEXW(E,setcc,1), IDEXW(E,setcc,1), EMPTY,
        /* 0x98 */ IDEXW(E,setcc,1), EMPTY, EMPTY, EMPTY,
        /* 0x9c */ IDEXW(E,setcc,1), EMPTY, IDEXW(E,setcc,1), IDEXW(E,setcc,1),
...
static make_EHelper(2byte_esc) {
  uint32_t opcode = instr_fetch(eip, 1) | 0x100; //+256
...
void exec_wrapper(bool print_flag) {
  ...
  decoding.p += sprintf(decoding.p, "%8x:   ", cpu.eip);  //save the eip?
#endif

  decoding.seq_eip = cpu.eip;  //save the eip!
  exec_real(&decoding.seq_eip);
//when exec_real returned, decoding.seq_eip will point to the addr of next inst.
#ifdef DEBUG
